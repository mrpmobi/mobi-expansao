...
const stepMeta: Record<string, { label: string; color: string; bg: string; border: string }> = {
  nao_iniciado: { label: "Não iniciado", color: "text-gray-500 dark:text-gray-400", bg: "bg-gray-100 dark:bg-gray-800", border: "border-gray-300 dark:border-gray-600" },
  semana_1: { label: "Semana 1 - Diagnóstico", color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-950", border: "border-blue-200 dark:border-blue-800" },
  semana_2: { label: "Semana 2 - Execução", color: "text-yellow-600 dark:text-yellow-400", bg: "bg-yellow-50 dark:bg-yellow-950", border: "border-yellow-200 dark:border-yellow-800" },
  semana_3: { label: "Semana 3 - Recuperação", color: "text-orange-600 dark:text-orange-400", bg: "bg-orange-50 dark:bg-orange-950", border: "border-orange-200 dark:border-orange-800" },
  semana_4: { label: "Semana 4 - Avaliação", color: "text-violet-600 dark:text-violet-400", bg: "bg-violet-50 dark:bg-violet-950", border: "border-violet-200 dark:border-violet-800" },
  finalizado: { label: "Finalizado", color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-950", border: "border-emerald-200 dark:border-emerald-800" },
}
    ...
    <div className="space-y-6">
                ...
                <div key={step} className="flex items-center gap-1 shrink-0">
                  <div
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border
                      ...
                      ${isPast ? "bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800" : ""}
                      ${isFuture ? "bg-gray-50 dark:bg-gray-800 text-gray-400 dark:text-gray-500 border-gray-200 dark:border-gray-700" : ""}
                    `}
                  >
                  ...
                  {i < programSteps.length - 1 && (
                    <ArrowRight className={`h-4 w-4 ${isPast ? "text-emerald-400 dark:text-emerald-500" : "text-gray-300 dark:text-gray-600"}`} />
                  )}
                </div>
              )
            })}
      ...
      {lider.programStatus === "finalizado" && (
        <Card className="border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-emerald-700 dark:text-emerald-400">Programa Finalizado</h3>
            <p className="text-sm text-emerald-600 dark:text-emerald-400 mt-1">
              Score: {lider.score}% &middot; Classificação: {lider.classificacao}
            </p>
            <p className="text-sm text-emerald-600 dark:text-emerald-400 mt-2 max-w-lg mx-auto">{lider.feedback}</p>
          </CardContent>
        </Card>
      )}
                ...
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 text-sm">
                    ...
                    {totalCorridasLider >= 300 ? (
                      <p className="text-emerald-700 dark:text-emerald-400 font-medium">Meta já atingida! ({formatNumber(totalCorridasLider)} corridas)</p>
                    ) : (
            ...
            {currentWeek.semana === 4 && (
              <Card className="border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950">
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 text-sm">
                    <p><strong>Meta atingida?</strong> {totalCorridasLider >= 300 ? <span className="text-emerald-600 dark:text-emerald-400 font-bold">SIM</span> : <span className="text-red-600 dark:text-red-400 font-bold">NÃO</span>}</p>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {currentWeek.checklist.map((item) => (
                  <label key={item.key} className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="checkbox" className="h-4 w-4 rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700" />
                    {item.label}
                  </label>
                ))}
              </div>
      ...
      {currentWeek && lider.semanas[currentWeekIdx]?.concluida && currentWeekIdx < 3 && (
        <div className="text-center p-4 bg-emerald-50 dark:bg-emerald-950 rounded-lg border border-emerald-200 dark:border-emerald-800">
          <p className="text-emerald-700 dark:text-emerald-400 font-medium">Semana {currentWeek.semana} concluída!</p>
          <p className="text-sm text-emerald-600 dark:text-emerald-400 mt-0.5">A Semana {currentWeek.semana + 1} já está disponível.</p>
        </div>
      )}
    </div>
  )
